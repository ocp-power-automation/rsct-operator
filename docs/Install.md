...
- Create a CatalogSource using the image `ghcr.io/ocp-power-automation/rsct-operator-catalog:v0.0.1-alpha2` in the `openshift-marketplace` namespace.
```
oc apply -f - <<EOF
...
spec:
  ...
  publisher: IBM
  sourceType: grpc
  image: ghcr.io/ocp-power-automation/rsct-operator-catalog:v0.0.1-alpha2
EOF
```
